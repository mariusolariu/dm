	...
	for (int d=0; d<num_docs; ++d) // Loop over documents
	{
		for (int w=0; w<num_words; ++w) // Loop over words
		{
			...
			*/
			...
			PzB[d][w] = (lambda_B * PB[w]) / ((lambda_B * PB[w]) + (1 - lambda_B) * res); 
...
void PLSA::Submit()
{
    ...
    submission.open("Assignment3/plsa.txt");
...
int main(int argc, char* argv[])
{
    ...
    auto idx = index::make_index<index::dblru_inverted_index>(*config, 30000); // Inverted Index

    auto fidx = meta::index::make_index<index::memory_forward_index>(*config); // Forward Index
